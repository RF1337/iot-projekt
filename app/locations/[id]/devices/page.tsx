    ...
    <div className="w-full">
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
        ...
        <div className="space-y-5">
          <div className="flex items-center justify-between rounded-2xl border border-violet-200 bg-violet-50/75 px-5 py-4">
            <h1 className="text-2xl font-bold text-violet-950">Dine Enheder</h1>
          ...
          {!loading && sensors.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-violet-300 bg-violet-50/80 py-16 text-center">
              <div className="mb-4 rounded-full bg-violet-100 p-4">
                <Cpu className="h-8 w-8 text-violet-700" />
              </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-violet-50">
                  <TableHead className="font-semibold text-violet-900">Enhedsnavn</TableHead>
                  <TableHead className="font-semibold text-violet-900">Type</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {loading ? (
                  ...
                  sensors.map((sensor) => (
                    <TableRow key={sensor.id} className="hover:bg-violet-50/60">
        ...
        {/* Right: setup guide */}
        <div className="rounded-xl border border-emerald-200 bg-emerald-50/80 p-8">
          <h2 className="mb-8 text-center text-xl font-bold text-emerald-950">Opsætningsguide</h2>
          <ol className="space-y-8">
            {SETUP_STEPS.map(({ step, title, description }) => (
              <li key={step} className="flex flex-col items-center text-center">
                <span className="mb-3 flex h-9 w-9 items-center justify-center rounded-full bg-emerald-600 text-sm font-bold text-white">
                  {step}
                </span>
                <p className="font-semibold text-emerald-950">{title}</p>
                <p className="mt-1 max-w-xs text-sm text-emerald-800/70">{description}</p>
              </li>
            ))}
          </ol>
        </div>
