...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="rounded-xl border border-teal-200 bg-teal-50 px-4 py-3 text-sm shadow-md">
      <p className="text-xs text-teal-600 mb-1">{dateStr} kl. {timeStr}</p>
...
export default function Dashboard() {
    ...
    try {
      ...
      if (dbData) {
        ...
        if (formatted.length > 0) {
          const latestUpdate = new Date(formatted[formatted.length - 1].iso);
          setLastReadingAt(latestUpdate);
        } else {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          setLastReadingAt(new Date(newRow.created_at));
          setData((prev) => [
            ...prev.slice(-1999),
  ...
  const getStatusColor = (status: string) =>
    status === "Online"
      ...
      ? "text-amber-600"
      : "text-red-600";
  const statusCardClassName = systemStatus.hasFailure
    ? "border-red-300 bg-red-50/90 ring-0 shadow-sm animate-pulse"
    : "border-emerald-200 bg-emerald-50/90 ring-0 shadow-sm";
  const statusHeadingClassName = systemStatus.hasFailure ? "text-red-700" : "text-emerald-700";
  const statusLabelClassName = systemStatus.hasFailure ? "text-red-950/70" : "text-emerald-900/70";
  const statusDividerClassName = systemStatus.hasFailure ? "border-red-200" : "border-emerald-200";
  const statusHintClassName = systemStatus.hasFailure ? "text-red-700/70" : "text-emerald-700/70";
    ...
    <div className="w-full space-y-6">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-1 rounded-xl bg-white/70 px-3 py-2 text-xl font-bold text-gray-900 shadow-sm ring-1 ring-teal-900/10 transition hover:bg-teal-50 hover:text-primary">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Card className="border-indigo-200 bg-indigo-50/90 ring-0 shadow-sm">
            <CardContent className="p-5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold uppercase tracking-wider text-indigo-700">Aktuel temperatur</span>
              </div>
              ...
              <div className="mt-3 flex items-center justify-between">
                <p className="text-xs text-indigo-700/70">Sidst opdateret: {systemStatus.device.lastUpdatedLabel}</p>
                {temperatureDelta !== null && (
                  <span className={`text-xs font-medium ${
                    temperatureDelta > 0 ? "text-orange-500" : temperatureDelta < 0 ? "text-cyan-600" : "text-gray-400"
                  }`}>
          ...
          <Card className="border-indigo-200 bg-indigo-50/90 ring-0 shadow-sm">
            <CardContent className="p-5">
              <span className="text-xs font-semibold uppercase tracking-wider text-indigo-700">Gennemsnit</span>
              <p className="mt-3 text-5xl font-bold tracking-tight text-gray-900">
                {averageTemperature !== null ? `${averageTemperature.toFixed(1)}°` : "--"}
                <span className="ml-1 text-2xl font-normal text-gray-400">C</span>
              </p>
              <p className="mt-3 text-xs text-indigo-700/70">Baseret på {data.length} målinger i perioden</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-indigo-200 bg-indigo-50/90 ring-0 shadow-sm">
            <CardContent className="p-5">
              <span className="text-xs font-semibold uppercase tracking-wider text-indigo-700">Min / Maks</span>
              <div className="mt-3 flex items-end gap-4">
                <div>
                  <p className="mb-1 text-xs font-medium text-cyan-600">MIN</p>
                  <p className="text-3xl font-bold tracking-tight text-gray-900">
                    {minTemperature !== null ? `${minTemperature.toFixed(1)}°` : "--"}
                  </p>
                </div>
                <span className="mb-1 text-2xl text-indigo-300 font-light">/</span>
                <div>
              ...
              <p className="mt-3 text-xs text-indigo-700/70">Inden for den valgte tidsperiode</p>
            </CardContent>
          </Card>
          ...
          <Card className={statusCardClassName}>
            <CardContent className="p-5">
              <span className={`text-xs font-semibold uppercase tracking-wider ${statusHeadingClassName}`}>Status</span>
              <div className="mt-3 space-y-2 text-sm">
                <div className="flex items-center justify-between gap-3">
                  <span className={statusLabelClassName}>Sensor/Arduino</span>
                  <span className={`flex items-center gap-1.5 font-semibold ${getStatusColor(systemStatus.device.status)}`}>
                    {systemStatus.device.status === "Online" ? <Wifi className="h-3.5 w-3.5" /> : <WifiOff className="h-3.5 w-3.5" />}
                    {systemStatus.device.status}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-3">
                  <span className={statusLabelClassName}>Client</span>
                  <span className={`flex items-center gap-1.5 font-semibold ${getStatusColor(systemStatus.browser.status)}`}>
                    {systemStatus.browser.online ? <Wifi className="h-3.5 w-3.5" /> : <WifiOff className="h-3.5 w-3.5" />}
                    {systemStatus.browser.status}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-3">
                  <span className={statusLabelClassName}>API</span>
                  <span className={`flex items-center gap-1.5 font-semibold ${getStatusColor(systemStatus.api.status)}`}>
                    {systemStatus.api.status === "Online" ? <Wifi className="h-3.5 w-3.5" /> : <WifiOff className="h-3.5 w-3.5" />}
                    {systemStatus.api.status}
                  </span>
                </div>
                <div className={`border-t pt-2 ${statusDividerClassName}`}>
                  <p className={`text-xs font-medium ${statusLabelClassName}`}>Last sent</p>
                  <p className="mt-1 text-sm font-semibold text-gray-900">{systemStatus.device.lastSentLabel}</p>
                </div>
              </div>
              <p className={`mt-3 text-xs ${statusHintClassName}`}>Sensor offline efter {OFFLINE_AFTER_SECONDS / 60} min uden en ny værdi</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="rounded-xl border border-sky-200 bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-base font-semibold text-sky-950">Temperaturgraf over tid</h2>
          ...
          {loading ? (
            ...
            <p className="py-10 text-center text-sm text-gray-500">Ingen sensorer på denne lokation.</p>
          ) : data.length === 0 ? (
